...
7. ✅ **Live Run Feedback** *(UX track)* — a running loop is now legible: the Loop Pipeline visibly advances with per‑phase progress + a **"phase n of 5"** bar, the active stage pulses, **EVALUATE** lights up as its own stage, and the latest streamed line + a ticking elapsed/token readout sit in a status strip under the pipeline (newest console line highlighted live).
8. ✅ **Information Architecture** *(UX track)* — the cockpit splits into two modes: an **ORG view** where the six top tabs aggregate across *all* loops (a fleet dashboard of department cards, org agent/task rollups, the cross‑loop artifact browser, analytics, settings), and a **LOOP view** — click any department to open its own workspace page (header, live pipeline, agents, task board, console) with a `← Org` breadcrumb back. The right **Inspector** is one scrolling, drag‑resizable, collapsible panel (Details/Config/History stacked; an org summary in ORG view), and **⌘N/⌘A/⌘T** open dedicated creation modals (New Loop → a persisted department) instead of the ⌘K search window.
9. ✅ **Members, Roles & Integrations** *(UX track)* — Settings reflects reality. **Members** is a real, SQLite‑persisted roster that starts from just your local commander: add (name/email/role), delete (guarded — never yourself or the last owner), and role changes are gated by the RBAC matrix (`canAssignRole`, no privilege escalation). **Integrations** tells the truth for a local‑first app: **Ollama** shows a live CONNECTED / NOT REACHABLE status (a real daemon ping) with its model count, **Claude** shows CONFIGURED once a key is set, and CMA / Temporal / Redis / Postgres read “NOT CONFIGURED · requires Docker/creds” — genuinely gated, not broken.
